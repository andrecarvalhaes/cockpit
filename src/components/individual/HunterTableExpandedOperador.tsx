...
export const HunterTableExpandedOperador: React.FC<HunterTableExpandedOperadorProps> = ({
  ...
  const getHeatMapColor = (phase: string, periodIndex: number): string => {
    ...
    const values = periodMetrics.map(pm =>
      getValueByPhase(pm.metrics, phase)
    );

    const minValue = Math.min(...values);
    const maxValue = Math.max(...values);
    const currentValue = getValueByPhase(periodMetrics[periodIndex].metrics, phase);
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {selectedPhases.length === 0 ? (
                <tr>
                  <td colSpan={periodMetrics.length + 1} className="px-6 py-8 text-center text-text-secondary">
